...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    //    expired, or doesn't match this block/name/amount.
    if (!_token || !_ts) {
      return NextResponse.json(
    ...
    // 3. Verify session-continuity key: HMAC(api_key, SECRET) must equal api_key_hash.
    //    This ensures the URL's api_key param was generated by THIS session's prepare call
    //    and hasn't been replayed from a previous transaction.
    if (!api_key || !api_key_hash) {
      return NextResponse.json(
        { success: false, error: "Invalid request: missing session continuity key." },
        { status: 401 }
      );
    }
    const expectedKeyHash = createHmac("sha256", SECRET).update(api_key.trim()).digest("hex");
    if (expectedKeyHash !== api_key_hash) {
      return NextResponse.json(
        { success: false, error: "Payment session mismatch. Please start a new payment." },
        { status: 401 }
      );
    }
    // ── End security gate ──────────────────────────────────────────────────────
