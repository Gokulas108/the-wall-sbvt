...
export async function GET(req: NextRequest) {
  const whatsappNumber =
    req.nextUrl.searchParams.get("whatsapp_number")?.trim() ??
  ...
  if (digitsOnly.length > 0 && digitsOnly.length !== whatsappNumber.length) {
  ...
  if (digitsOnly.length >= 2 && digitsOnly.length <= 15) {
    const sortedCodes = [...COUNTRY_CALLING_CODES].sort(
      (a, b) => b.length - a.length,
    );
    const matchedCode = sortedCodes.find((code) => digitsOnly.startsWith(code));
    if (matchedCode && digitsOnly.length > matchedCode.length) {
      normalizedNumber = `+${matchedCode} ${digitsOnly.slice(matchedCode.length)}`;
    }
  }
