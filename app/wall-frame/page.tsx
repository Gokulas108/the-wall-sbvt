...
const CornerOrnament = ({ className }: { className?: string }) => (
  ...
  >
    ...
    <path
      d="M6 6H38C38 23.6731 23.6731 38 6 38V6Z"
      fill="url(#goldGradient)"
      fillOpacity="0.5"
    />
    <path
      d="M11 11H26C26 19.2843 19.2843 26 11 26V11Z"
      fill="url(#goldGradient)"
    />
...
export default function WallFramePage() {
  ...
  const [mobileTab, setMobileTab] = useState<"new-donor" | "view-donation">(
    "new-donor",
  );
  ...
  const [highlightSubmissions, setHighlightSubmissions] = useState<
    SubmissionDetail[]
  >([]);
  const [highlightSubmissionsLoading, setHighlightSubmissionsLoading] =
    useState(false);
  const [receiptOverlay, setReceiptOverlay] = useState<SubmissionDetail | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/blocks/${focusedBlock}/submissions?name=${encodeURIComponent(highlightName)}`,
    )
      .then((r) => r.json())
      ...
      .catch(() => {
        if (!cancelled) setHighlightSubmissions([]);
      })
      .finally(() => {
        if (!cancelled) setHighlightSubmissionsLoading(false);
      });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    blockSwitchingRef.current = blockSwitching;
  }, [blockSwitching]);
  const blocksLookupRef = useRef(blocks);
  useEffect(() => {
    blocksLookupRef.current = blocks;
  }, [blocks]);
      ...
      try {
        if (serial) {
          const res = await fetch(
            `/api/search?q=${encodeURIComponent(serial)}`,
          );
          const data = (await res.json()) as { results?: SearchItem[] };
          const hit =
            (data.results ?? []).find(
              (r) => r.serial_number?.toLowerCase() === serial.toLowerCase(),
            ) ?? (data.results ?? [])[0];
  ...
  function validateFormFields(
    ...
    if (phCountry) {
      const d = formPhone.replace(/\D/g, "").length;
      if (d < phCountry.min || d > phCountry.max) {
        setFormStatus(
          `Mobile number must be ${phCountry.min === phCountry.max ? phCountry.min : `${phCountry.min}-${phCountry.max}`} digits.`,
        );
        return false;
      }
    }
    ...
    if (!formSamePhone) {
      const waC = COUNTRY_CODES.find((c) => c.code === formWaCode);
      if (waC) {
        const d = wa.replace(/\D/g, "").length;
        if (d < waC.min || d > waC.max) {
          setFormStatus(
            `WhatsApp must be ${waC.min === waC.max ? waC.min : `${waC.min}-${waC.max}`} digits.`,
          );
          return false;
        }
      }
  ...
  async function handlePayOnline() {
    ...
    try {
      ...
      if (!pendingData.success || !pendingData.api_key_hash) {
        const msg = pendingData.error ?? "Could not initiate payment.";
        setPaymentError(msg);
        setFormStatus(msg);
        return;
      }
  ...
  useEffect(() => {
    ...
    blocks.forEach((b, id) => {
      if (b.remaining > 0) available.push(id);
    });
    ...
    >
              ...
              {focusedBlock && focusData && highlightName ? (
                ...
                >
                  <button
                    ...
                    className="self-start inline-flex items-center gap-2 px-3 py-2 rounded-md text-[11px] font-bold tracking-widest uppercase transition-all hover:brightness-110 active:scale-[0.99]"
                    style={{
                      background: "rgba(255,246,233,0.08)",
                      border: "1px solid rgba(228,180,121,0.28)",
                      color: "#ffe9cc",
                  ...
                  {(() => {
                    const totalQty = highlightSubmissions.reduce(
                      (sum, s) => sum + (s.qty || 0),
                      0,
                    );
                    const totalAmt = highlightSubmissions.reduce(
                    ...
                    return (
                      <div
                        ...
                        style={{
                          background:
                            "linear-gradient(180deg, rgba(40,8,12,0.95), rgba(20,4,6,0.98))",
                          border: "1px solid rgba(215,173,87,0.3)",
                          boxShadow:
                            "0 18px 48px rgba(0,0,0,0.45), inset 0 1px 0 rgba(252,234,187,0.08)",
                        }}
                      >
                        <div className="flex items-center justify-between">
                          <p
                            className="text-xs font-bold tracking-wider uppercase"
                            style={{ color: "rgba(215,173,87,0.9)" }}
                          >
                            Inscription Details
                          </p>
                          {highlightSubmissions.length > 1 && (
                            ...
                            >
                              {highlightSubmissions.length} donations
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex flex-col gap-2">
                          <div
                            ...
                            style={{
                              ...
                              border: "1px solid rgba(228,180,121,0.18)",
                            }}
                          >
                            <span
                            ...
                            <span
                              className="text-sm font-extrabold text-right break-words"
                              style={{ color: "#fff5e7" }}
                            >
                              {highlightName}
                            </span>
                          ...
                          <div
                            className="flex justify-between items-center gap-3 px-3 py-2 rounded-lg"
                            style={{
                              background: "rgba(255,246,233,0.06)",
                              border: "1px solid rgba(228,180,121,0.18)",
                            }}
                          ...
                          <div
                            className="flex justify-between items-center gap-3 px-3 py-2 rounded-lg"
                            style={{
                              background: "rgba(215,173,87,0.12)",
                              border: "1px solid rgba(215,173,87,0.28)",
                            }}
                          >
                            <span
                              className="text-[10px] font-bold tracking-wider uppercase"
                              style={{ color: "rgba(215,173,87,0.95)" }}
                            >
                              Amount Donated
                            </span>
                            <span
                        ...
                        {highlightSubmissionsLoading &&
                          highlightSubmissions.length === 0 && (
                            <p
                        ...
                        {highlightSubmissions.length === 0 &&
                          !highlightSubmissionsLoading && (
                            <p
                              ...
                              style={{ color: "rgba(215,173,87,0.7)" }}
                            >
                              No detailed donation record on file.
                            </p>
                          )}
                          ...
                          return (
                            ...
                            >
                              <div className="flex justify-between items-center gap-2">
                                ...
                                <span
                                  className="text-xs font-bold break-all text-right"
                                  style={{ color: "#fceabb" }}
                                >
                                  {s.serial_number ?? "—"}
                                </span>
                              ...
                              <button
                                ...
                                style={{
                                  background:
                                    "linear-gradient(135deg, #d77a26 0%, #8a3a0a 60%, #5a2308 100%)",
                                  color: "#fff5e7",
                                  border: "1px solid rgba(252,234,187,0.5)",
                                  boxShadow:
                                    "0 6px 14px rgba(201,107,27,0.4), inset 0 1px 0 rgba(252,234,187,0.4)",
                                  fontFamily: '"Cinzel", Georgia, serif',
                              ...
                              <a
                                ...
                                style={{
                                  background:
                                    "linear-gradient(135deg, #b08124 0%, #6b4a12 60%, #3f2a08 100%)",
                                  color: "#fff5e7",
                                  border: "1px solid rgba(252,234,187,0.5)",
                                  boxShadow:
                                    "0 6px 14px rgba(176,129,36,0.4), inset 0 1px 0 rgba(252,234,187,0.4)",
                                  fontFamily: '"Cinzel", Georgia, serif',
                                  textShadow: "0 1px 2px rgba(0,0,0,0.5)",
                                }}
                              >
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      <div
                        className="px-3 py-1.5 rounded-md flex flex-col items-center"
                        style={{
                          background:
                            "linear-gradient(135deg, rgba(201,107,27,0.35), rgba(138,90,25,0.2))",
                          border: "1px solid rgba(252,234,187,0.45)",
                          boxShadow:
                            "0 0 18px rgba(201,107,27,0.45), inset 0 0 12px rgba(252,234,187,0.15)",
                        }}
                      >
                        <span
                          className="text-[9px] font-bold uppercase tracking-[0.25em] leading-none mb-1 text-center"
                          style={{ color: "rgba(252,234,187,0.75)" }}
                        >
                          Block
                        </span>
                        <h2
                          className="text-3xl lg:text-4xl font-black leading-none text-center"
                          style={{
                            fontFamily: '"Cinzel", Georgia, serif',
                            background:
                              "linear-gradient(135deg, #fff5e7, #fceabb, #d7ad57)",
                            WebkitBackgroundClip: "text",
                            color: "transparent",
                            textShadow: "0 0 24px rgba(252,234,187,0.6)",
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        >
                          Used:{" "}
                          <span style={{ color: "#fceabb" }}>
                            {focusData.total_used}
                          </span>
                        </span>
                        <span
                          className="text-[10px] font-bold uppercase tracking-wider"
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="flex gap-2">
                        <select
                          ...
                          className="w-20 px-1 py-2 rounded-lg text-xs outline-none"
                          style={{
                            background: "rgba(255,250,244,0.96)",
                            border: "1px solid rgba(222,182,131,0.36)",
                            color: "#2a1509",
                        ...
                        >
                          {COUNTRY_CODES.map((c) => (
                            <option key={`ph-${c.name}`} value={c.code}>
                              {c.name} ({c.code})
                            </option>
                          ))}
                        </select>
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        onChange={(e) =>
                          handleSamePhoneChange(e.target.checked)
                        }
                        className="w-3.5 h-3.5"
                      />
                      <span
                    ...
                    {!formSamePhone && (
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="flex gap-2">
                          ...
                          >
                            {COUNTRY_CODES.map((c) => (
                              <option key={`wf-${c.name}`} value={c.code}>
                                {c.name} ({c.code})
                              </option>
                            ))}
                          </select>
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      >
                        Date of Birth{" "}
                        <span style={{ color: "rgba(255,230,198,0.5)" }}>
                          (optional)
                        </span>
                      </label>
                      <input
                    ...
                    <div
                      className="flex items-start gap-2 px-3 py-2 rounded-lg text-[10px] leading-snug"
                      style={{
                    ...
                    <button
                      className="w-full py-2.5 rounded-lg text-sm font-bold tracking-wider uppercase transition-all hover:brightness-110 active:scale-[0.99]"
                      style={{
                        background:
                          "linear-gradient(135deg, #d77a26 0%, #8a3a0a 60%, #5a2308 100%)",
                        color: "#fff5e7",
                        border: "1px solid rgba(252,234,187,0.5)",
                        boxShadow:
                          "0 8px 20px rgba(201,107,27,0.45), inset 0 1px 0 rgba(252,234,187,0.4)",
                        fontFamily: '"Cinzel", Georgia, serif',
                        textShadow: "0 1px 2px rgba(0,0,0,0.5)",
                ...
                >
                  <div className="shrink-0 mb-6 mt-4 text-center">
                    <div className="flex justify-center mb-4">
                      ...
                      >
                        <path
                          d="M30 0L35 7.5L30 15L25 7.5L30 0Z"
                          fill="#d7ad57"
                          opacity="0.8"
                        />
                        <path
                          d="M0 7.5H20M40 7.5H60"
                          stroke="#d7ad57"
                          strokeOpacity="0.4"
                          strokeWidth="1"
                        />
                    ...
                    <p
                      className="text-sm font-semibold tracking-[0.15em] mb-2"
                      style={{
                        color: "#d7ad57",
                    ...
                    <h1
                      ...
                      style={{
                        fontFamily: '"Cinzel", Georgia, serif',
                        background:
                          "linear-gradient(135deg, #fceabb, #d7ad57, #c96b1b)",
                        WebkitBackgroundClip: "text",
                        color: "transparent",
                  ...
                  <div className="flex-1 flex flex-col min-h-0 mb-4">
                    <label
                      className="shrink-0 block text-xs font-bold tracking-wider uppercase mb-3 text-center"
                      style={{ color: "#a88e6f" }}
                    >
                      Find Inscription
                    </label>
                    <div className="shrink-0 relative">
                      ...
                      />
                      <svg
                        className="w-4 h-4 absolute right-4 top-1/2 -translate-y-1/2 text-[#d7ad57] pointer-events-none"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                    ...
                    <div className="shrink-0 flex gap-2 mt-2">
                      <button
                        ...
                        style={{
                          background:
                            "linear-gradient(135deg, #c96b1b, #8a5a19)",
                          color: "#fff5e7",
                    ...
                    <p
                      className="shrink-0 text-[10px] mt-2 text-center"
                    ...
                    {searchResults.length > 0 && (
                      <div className="relative flex-1 min-h-0 mt-3">
                        <div className="absolute inset-0 overflow-y-auto space-y-2 pr-1 pb-6">
                          {searchResults.map((result, index) => (
                            <button
                              ...
                              type="button"
                              onClick={() =>
                                void openBlock(result.block_id, result.label)
                              }
                        ...
                        {searchResults.length > 4 && (
                          <>
                            <div
                              className="pointer-events-none absolute bottom-0 left-0 right-1 h-10 rounded-b-lg"
                              style={{
                                background:
                                  "linear-gradient(to top, rgba(50,10,15,0.98), rgba(50,10,15,0.6) 50%, transparent)",
                              }}
                            />
                            <div className="pointer-events-none absolute bottom-1 left-0 right-0 flex justify-center">
                              <div
                                ...
                                style={{
                                  ...
                                  border: "1px solid rgba(252,234,187,0.25)",
                                }}
                              >
                                <span
                    ...
                    {!searchLoading &&
                      ...
                      searchQuery.trim().length >= 2 && (
                        <div
                          className="shrink-0 mt-3 rounded-lg px-3 py-2 text-xs text-center"
                          style={{
                            background: "rgba(255,246,233,0.06)",
                            border: "1px solid rgba(228,180,121,0.2)",
                            color: "#ffe9cc",
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden">
        ...
        {!isBooting && !focusedBlock && (
          ...
          >
            <div
              className="relative z-10 flex w-full max-w-sm p-1 rounded-lg"
              style={{
                background: "rgba(60, 15, 20, 0.9)",
        ...
        <AnimatePresence>
          {hoveredBlock && !focusedBlock && !isBooting && (
            ...
            >
              ...
              <span className="text-[#a88e6f] text-xs uppercase tracking-widest font-bold">
                Focusing
              </span>
              <span className="text-[#fceabb] font-bold text-xl font-serif tracking-widest">
                {hoveredBlock}
              </span>
            </motion.div>
          )}
        ...
        {!isBooting && mobileTab === "view-donation" && !focusedBlock && (
          ...
          >
            <div
              className="p-6 rounded-2xl shadow-2xl relative overflow-hidden border border-[#c96b1b]/30"
              style={{
              ...
              <div className="relative z-10">
                ...
                <div className="mb-8">
                  <div className="relative">
                    ...
                    />
                    <svg
                      className="w-5 h-5 absolute right-4 top-1/2 -translate-y-1/2 text-[#d7ad57] pointer-events-none"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
          ...
          <div className="flex-1 w-full flex flex-col overflow-y-auto md:hidden px-4 pt-4 pb-8 gap-4">
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <p
                    ...
                    style={{ color: "rgba(66,44,25,0.7)" }}
                  >
                    In Focus · Block {focusedBlock}
                  </p>
                  <h2
                ...
                <button
                  ...
                  style={{
                    background:
                      "linear-gradient(180deg, rgba(50,10,15,0.95), rgba(25,5,8,0.98))",
                    color: "#e6c18a",
                    fontFamily: '"Cinzel", Georgia, serif',
                    border: "1px solid rgba(215,173,87,0.4)",
                  }}
                >
                  ←{" "}
            ...
            {highlightName && (
              <div
                ...
                style={{
                  aspectRatio: "1/1",
                  minHeight: "min(65vh, 92vw)",
                  background: "#0a0604",
                  boxShadow:
                    "0 20px 60px rgba(0,0,0,0.3), 0 0 0 1px rgba(201,107,27,0.3)",
                }}
              >
                <WallFrame
                  blocks={blocks}
                  singleBlockId={focusedBlock}
                      ...
                      >
                        ...
                        <div className="flex justify-between items-center gap-2">
                          ...
                          <span
                            ...
                            style={{ color: "#fff5e7" }}
                          >
                            {s.action_type === "pledge" ? "Pledge" : "Donation"}
                          </span>
                        </div>
                        <div className="flex justify-between items-center gap-2">
                          <span
              ...
              >
                ...
                <div className="flex gap-3">
                  <div className="flex-1 flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      maxLength={40}
                      placeholder="Donor name"
                      className="w-full px-3 py-3 rounded-lg outline-none focus:ring-2"
                      style={{
                  ...
                  <div className="w-20 flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-3 rounded-lg outline-none text-center font-bold"
                      style={{
                        background: "rgba(255,250,244,0.98)",
                        border: "1px solid rgba(222,182,131,0.4)",
                        color: "#2a1509",
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="flex gap-2">
                    <select
                      value={formPhoneCode}
                      onChange={(e) => setFormPhoneCode(e.target.value)}
                      className="w-24 px-2 py-3 rounded-lg outline-none font-semibold"
                      style={{
                        background: "rgba(255,250,244,0.98)",
                        border: "1px solid rgba(222,182,131,0.4)",
                        color: "#2a1509",
                    ...
                    >
                      {COUNTRY_CODES.map((c) => (
                        <option key={`mph-${c.name}`} value={c.code}>
                          {c.name} ({c.code})
                        </option>
                      ))}
                    </select>
                ...
                >
                  <input
                    ...
                    className="w-4 h-4 accent-[#c96b1b]"
                  />
                  <span
                    className="text-xs font-semibold"
                    style={{ color: "#ffe2b6" }}
                ...
                {!formSamePhone && (
                  <div className="flex flex-col gap-1.5">
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        {COUNTRY_CODES.map((c) => (
                          <option key={`mwa-${c.name}`} value={c.code}>
                            {c.name} ({c.code})
                          </option>
                        ))}
                      </select>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  >
                    Date of Birth{" "}
                    <span style={{ color: "rgba(215,173,87,0.55)" }}>
                      (optional)
                    </span>
                  </label>
                  <input
                ...
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg, rgba(201,107,27,0.18), rgba(138,90,25,0.1))",
                    border: "1px solid rgba(252,234,187,0.3)",
                    boxShadow: "inset 0 1px 0 rgba(252,234,187,0.12)",
                  }}
                >
                  <span
                    className="text-[11px] font-bold tracking-[0.2em] uppercase"
                    style={{ color: "rgba(252,234,187,0.9)" }}
                  >
                    Total
                  </span>
                  <span
                ...
                {formStatus && (
                  <p className="text-xs" style={{ color: "#f6d8af" }}>
                    {formStatus}
                  </p>
                )}
              </div>
        ...
        <div
          className={`flex-1 w-full relative flex-col items-center justify-start overflow-y-auto md:overflow-hidden ${focusedBlock || mobileTab !== "new-donor" ? "hidden md:flex" : "flex mt-[70px]"} md:mt-0 pb-8 md:pb-0`}
        >
          ...
          <div className="flex-1 w-full flex flex-col items-center">
            ...
            {!isBooting && (
              <motion.div
                className="relative left-0 right-0 px-4 md:px-12 flex flex-col md:flex-row justify-between items-center z-10 gap-2 md:gap-3 py-6 md:py-4 md:mt-4 w-full"
                initial={{ opacity: 0, y: -15 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.8, delay: 0.3 }}
              >
                <div
                  className={`${highlightName ? "hidden md:hidden" : "flex md:flex"} items-center gap-3 w-full justify-center md:justify-start`}
                >
                  <svg
                    className="hidden md:block w-5 h-5 shrink-0"
                    style={{ color: "#c96b1b" }}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  ...
                  <p
                    className="text-[11px] md:text-base font-bold tracking-widest text-center md:text-left leading-relaxed"
                    style={{
                      color: "#3a1c0d",
                      fontFamily: '"Cinzel", Georgia, serif',
                      textShadow:
                        "0 1px 12px rgba(255,255,255,0.9), 0 0 4px rgba(255,255,255,1)",
                    }}
                  >
                    {focusedBlock ? (
                      <>
                        Block{" "}
                        <span
                          className="text-sm md:text-xl font-black align-baseline"
                          style={{ color: "#8a3a0a", letterSpacing: "0.04em" }}
                        >
                          {focusedBlock}
                        </span>{" "}
                        is selected for you to inscribe your name on
                      </>
                    ) : (
                ...
                {focusedBlock ? (
                  !highlightName && (
                    <button
                      onClick={closeBlock}
                      className="hidden md:inline-flex items-center gap-2 px-6 py-2 rounded-md text-xs font-bold tracking-widest uppercase transition-all shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 whitespace-nowrap"
                      style={{
                        background:
                          "linear-gradient(180deg, rgba(50,10,15,0.95), rgba(25,5,8,0.98))",
                        color: "#e6c18a",
                  ...
                  <button
                    ...
                    className="hidden md:block px-6 py-2 rounded-md text-xs font-bold tracking-widest uppercase transition-all shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 whitespace-nowrap"
                    style={{
                      background:
                        "linear-gradient(180deg, rgba(50,10,15,0.95), rgba(25,5,8,0.98))",
                      color: "#e6c18a",
            ...
            {dataReady && (
              <motion.div
                className="relative w-[min(80vh,90vw)] md:w-[min(75vh,calc(100vw-30rem))] mt-2 md:mt-4"
              ...
              >
                {(() => {
                  ...
                  const fr = focusedBlock
                    ? parseInt(focusedBlock.slice(1), 10)
                    : null;
                  const activePill: React.CSSProperties = {
                    ...
                    boxShadow:
                      "0 0 10px rgba(201,107,27,0.85), 0 2px 4px rgba(0,0,0,0.4)",
                    fontWeight: 900,
                    fontFamily: '"Cinzel", Georgia, serif',
                  ...
                  const inactiveLabel: React.CSSProperties = {
                    color: "rgba(58,28,13,0.85)",
                  ...
                  return (
                    <>
                      ...
                      <div
                        className="absolute left-0 right-0 grid z-10"
                        style={{
                          bottom: "100%",
                          height: "1.6rem",
                          marginBottom: "0.2rem",
                      ...
                      >
                        {COL_LABELS.map((label, i) => {
                          ...
                          return (
                            ...
                            >
                              <span style={active ? activePill : inactiveLabel}>
                                {label}
                              </span>
                            </motion.span>
                          );
                      ...
                      <div
                        className="absolute top-0 bottom-0 grid z-10"
                        style={{
                          right: "100%",
                          width: "2.2rem",
                          marginRight: "0.2rem",
                ...
                <div
                  className="w-full h-full relative z-20 rounded-xl overflow-hidden animate-gold-bg p-2.5 md:p-6"
                  style={{
                    boxShadow:
                      "0 40px 80px -10px rgba(0,0,0,0.7), 0 0 50px rgba(215,173,87,0.4), inset 0 0 12px rgba(255,255,255,0.6), inset 0 0 25px rgba(0,0,0,0.9)",
                    isolation: "isolate",
                ...
                >
                  <div className="absolute inset-0 overflow-hidden pointer-events-none z-10 rounded-xl">
                    <div
                      className="absolute top-0 bottom-0 w-[50%] animate-gold-sheen"
                      style={{
                        background:
                          "linear-gradient(105deg, transparent 30%, rgba(255,255,255,0.1) 45%, rgba(255,255,255,0.3) 50%, rgba(255,255,255,0.1) 55%, transparent 70%)",
                      }}
                    />
                  </div>
                  <div
                    ...
                    style={{
                      backgroundImage: FRAME_PATTERN,
                      mixBlendMode: "overlay",
                    }}
                  />
                  <div
                    className="w-full h-full relative p-1 md:p-2 rounded z-20"
                    style={{
                      background:
                        "linear-gradient(to bottom right, rgba(0,0,0,0.6), rgba(0,0,0,0.3))",
                      boxShadow:
                        "inset 0 0 10px rgba(0,0,0,0.8), 0 0 5px rgba(255,255,255,0.2)",
                    }}
                  >
                    ...
                    <div
                      className="w-full h-full rounded-sm overflow-hidden relative z-10"
                      style={{
                        border:
                          "var(--wall-frame-border) solid rgba(24,12,6,0.95)",
                        boxShadow:
                          "inset 0 0 var(--wall-frame-shadow) rgba(0,0,0,0.9), 0 0 15px rgba(0,0,0,0.8)",
                      }}
                    >
                      <WallFrame
                        blocks={blocks}
                        singleBlockId={highlightName ? focusedBlock : null}
                        selectedBlockId={highlightName ? null : focusedBlock}
                        highlightName={highlightName}
                        className={`w-full h-full pointer-events-none md:pointer-events-auto ${blockSwitching ? "md:!pointer-events-none" : ""}`}
                        onBlockClick={handleWallBlockClick}
                        onBlockHover={handleWallBlockHover}
                        onFirstPaint={handleWallFirstPaint}
                      />
                      <AnimatePresence>
                        {blockSwitching && (
                          <motion.div
                          ...
                          >
                            <div
                              ...
                              style={{
                                ...
                                border: "1px solid rgba(215,173,87,0.45)",
                            ...
                            >
                              <div
                                ...
                                style={{
                                  borderColor: "rgba(252,234,187,0.25)",
                                  borderTopColor: "#fceabb",
                                }}
                              />
                              <span
                      ...
                      <AnimatePresence>
                        {mobileCol && (
                          <motion.div
                            className="absolute top-0 bottom-0 pointer-events-none z-20 md:hidden"
                            style={{
                              width: `${100 / GRID_SIZE}%`,
                              left: 0,
                              background: "rgba(252,234,187,0.15)",
                              borderLeft: "2px solid rgba(252,234,187,0.6)",
                              borderRight: "2px solid rgba(252,234,187,0.6)",
                            }}
                            initial={{
                              opacity: 0,
                              x: `${COL_LABELS.indexOf(mobileCol) * 100}%`,
                            }}
                            animate={{
                              opacity: 1,
                              x: `${COL_LABELS.indexOf(mobileCol) * 100}%`,
                        ...
                        {mobileRow && (
                          <motion.div
                            ...
                            style={{
                              ...
                              top: 0,
                              background: "rgba(252,234,187,0.15)",
                              borderTop: "2px solid rgba(252,234,187,0.6)",
                              borderBottom: "2px solid rgba(252,234,187,0.6)",
                            }}
                            initial={{
                              opacity: 0,
                              y: `${(mobileRow - 1) * 100}%`,
                      ...
                      <AnimatePresence>
                        {mobileCol && mobileRow && (
                          <motion.div
                            className="absolute pointer-events-none z-30 border-2 md:hidden"
                            style={{
                              width: `${100 / GRID_SIZE}%`,
                              height: `${100 / GRID_SIZE}%`,
                              left: 0,
                              ...
                              borderColor: "rgba(252,234,187,1)",
                              boxShadow:
                                "0 0 20px rgba(252,234,187,0.8), inset 0 0 10px rgba(252,234,187,0.4)",
                            }}
                            initial={{
                              opacity: 0,
                              scale: 0.8,
            ...
            {!isBooting && (
              <motion.div
                className="mt-3 z-10 w-full px-6 flex flex-col items-center"
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                {!highlightName && (
                  <p
                    className="hidden md:block text-sm tracking-widest uppercase font-bold"
                    style={{
                      color: "#3a1c0d",
                      fontFamily: '"Cinzel", Georgia, serif',
                  ...
                  >
                    {focusedBlock ? (
                      <>
                        Block{" "}
                        <span style={{ color: "#8a3a0a" }}>{focusedBlock}</span>{" "}
                        is selected. Click on any other block to change
                        selection
                      </>
                    ) : (
                ...
                <div className="md:hidden flex flex-col items-center w-full max-w-sm mx-auto min-h-[120px]">
                  <p
                    className="text-xs mb-3 tracking-[0.15em] uppercase font-black"
                    style={{
                      color: "#3a1c0d",
                      fontFamily: '"Cinzel", Georgia, serif',
                    }}
                  >
                    {mobileCol && mobileRow
                      ? `Your name will appear in block ${mobileCol}${mobileRow}`
                      : "Select block below"}
                  </p>
                  <div className="flex w-full gap-3">
                    <select
                      className="flex-1 bg-[#2a060a]/90 border border-[#c58c38]/50 rounded px-3 py-2.5 text-xs font-bold text-[#ffe2b6] focus:outline-none focus:border-[#fceabb] shadow-inner appearance-none"
                      value={mobileCol || ""}
                      onChange={(e) => {
                        setMobileCol(e.target.value || null);
                        setMobileRow(null);
                      }}
                      style={{
                        backgroundImage: `url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%23fceabb' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3e%3cpolyline points='6 9 12 15 18 9'%3e%3c/polyline%3e%3c/svg%3e")`,
                    ...
                    <select
                      className="flex-1 bg-[#2a060a]/90 border border-[#c58c38]/50 rounded px-3 py-2.5 text-xs font-bold text-[#ffe2b6] focus:outline-none focus:border-[#fceabb] shadow-inner appearance-none disabled:opacity-50"
                      value={mobileRow || ""}
                      onChange={(e) =>
                        setMobileRow(
                    ...
                    {(mobileCol || mobileRow) &&
                      (() => {
                        ...
                        const candidateBlock = candidateId
                          ? blocks.get(candidateId)
                          : null;
                        const isFull =
                          !!candidateBlock && candidateBlock.remaining <= 0;
                          ...
                          >
                            {isFull && (
                              <p
                                ...
                                style={{ color: "#c96b1b" }}
                              >
                                Block {candidateId} is full. Pick another block.
                              </p>
                            )}
                            <button
                              className="w-full py-2.5 rounded-md text-[10px] font-bold tracking-widest uppercase shadow-lg transition-all disabled:opacity-75 disabled:cursor-not-allowed"
                              disabled={!ready}
                              style={{
                                background: ready
                                  ? "linear-gradient(135deg, #c96b1b, #8a5a19)"
                                  : "rgba(60,15,20,0.8)",
                                color: ready ? "#fff5e7" : "#d7ad57",
                                border: ready
                                  ? "1px solid rgba(255,218,159,0.3)"
                                  : "1px solid rgba(215,173,87,0.3)",
                              }}
                              onClick={() => {
                                if (ready && candidateId) {
                                  void openBlock(candidateId);
                                  setMobileCol(null);
                                  setMobileRow(null);
                                }
                              }}
        ...
        {receiptOverlay && (
          ...
          >
            ...
            >
              ...
              <h2
                className="text-4xl sm:text-5xl mt-2 pb-4"
                style={{
                  color: "#ffd79c",
                  fontFamily: '"Dancing Script", cursive',
                  ...
                  <div className="grid sm:grid-cols-2 gap-3 mt-5">
                    ...
                    <div>
                      ...
                      <p style={{ color: "#fff4e3" }}>
                        {new Date(receiptOverlay.created_at).toLocaleString(
                          "en-IN",
                        )}
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <p style={{ color: "#fff4e3" }}>
                        ₹{formatINR(receiptOverlay.amount)}
                      </p>
                    </div>
                    <div>
                      <p
                      ...
                      <p style={{ color: "#fff4e3" }}>
                        {fmtMethodFull(receiptOverlay.payment_method)}
                      </p>
                    </div>
                    {focusedBlock && (
                      <div>
    ...
    return (
      <div className="flex-1 flex items-center justify-center h-full">
        <span
          ...
          style={{ color: "rgba(58, 28, 13, 0.6)" }}
        >
          Awaiting Today&apos;s First Inscription
        </span>
      </div>
    );
  }
  ...
  return (
    <>
      <div
        ...
        style={{
          background:
            "linear-gradient(to right, rgba(201,107,27,0.15), rgba(201,107,27,0.05))",
          color: "#c96b1b",
